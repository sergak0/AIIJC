# [AIIJC](https://aiijc.com/ru/). Финальная задача

## Задача
Задачей финала AIIJC в нашем треке было перевод условий задач по матемитики из одной темы в другую. Например, есть задача "Лошадь бегает со скоростью 60 км/ч. За сколько времени она пробежит 120 км?". Это задачка про животных, и нам нужно изменить текст условия, чтобы смысловая нагрузка не поменялась. Допустим, если мы хотим сменить тему данной задачки с "животные" на "литература", то наше условие будет выглядит как-то так : "Машинистка набирает текст со скоростью 60 символов/минуту. За сколько времени она наберет 120 слов?". И нам нужно реализовать преобразование для тем: животные, литература, спорт, музыка

Финальное апи для перевода тем можно найти [тут](https://github.com/sergak0/convert-theme-aiijc)
...
# AIIJC. Классификация
## Задача
Разработка автоматизированного решения, позволяющего классифицировать школьные задачи по интересам конкретного ученика.
