...
эмбеддингами (использовали navec).

Всех действуйщих лиц предварительно нашли при помощи баесовской модели (см раздел про выделение ключевых слов)
...
И мы домножали эти вероятности на расстояние (косинусное) от данного слова до темы,
в к которую хотим перевести текст. Косинусное расстояние и вероятности мы предварительно нормировали в (0, 1).
...
И это, в принципе, логично, так как он обучался, чтобы выдавать один классный ответ.
А следовательно у нас очень мало слов, из которых можно выбирать и нужной нам темы в них зачастую нет
* Вторая проблема заключалась в том, что если мы убираем все ключевые слова, то ему просто незачто зацепиться.
Например, при попытке предсказать маски для `У Миши было 3 [MASK], 2 из них он отдал Даше. Сколько [MASK] у него осталось?`
BERT теряется и предсказывает какую-то несуразицы
...
В данном подходе мы предсказывали дополнения при помощи косинусных расстояний.
При помощи векторов слов, которые стояли на этом месте до нас и информации о 
действующем лице (которое мы подставили ранее), мы могли выбрать наилучшего кандидата 
из списка ключевых слов для целевой темы.

** Тут рассказано о том, вектора каких слов скадвали и вычитали **

** Тут о о том, как выделяли ключевыю слова из naveca **
...
Для того, чтобы согласовать все падежи, роды, склонения и тд, которые стоят неправильно 
в связи с использованием начальных форм слов, мы переводили текст на английский, а потом обратно на русский.

Для перевода использовали https://www.m-translate.ru/translator/text#text=test&direction=ru-en. 
Так как он не имеет ограничения на количество запросов
# AIIJC. Классификация
